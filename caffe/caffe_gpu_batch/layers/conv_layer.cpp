...
int 	net_conv_initial_para(t_layer_info*  layer_list,int layer_id,int in_ch)
{
	int para_size;
	...
	if(layer_list[layer_id].train_inf.para_diff_way==e_para1_prune){
		...
		int channel_size=conv_w_size/o_c;
		...
		float* para1_data_host=new float[conv_w_size];
		cuda_pull_array(layer_list[layer_id].para1_entry, para1_data_host, conv_w_size);
		caffe_mul(conv_w_size, para1_data_host, layer_list[layer_id].train_inf.para_diff_mask, para1_data_host);
		cuda_push_array(layer_list[layer_id].para1_entry, para1_data_host, conv_w_size);
...
int 	net_conv_forward(t_layer_info*  layer_list,int layer_id, float* bottom, int ch, int h, int w,float* top)
{
...
int net_conv_forward_batch(t_layer_info*  layer_list,int layer_id,int batch_sz)
{
	...
	for(int i=0;i<batch_sz;i++){
		...
		//	print_stop_sign("conv_layer\n");
		//	}

		net_conv_forward( layer_list, layer_id, bottom_, i_c, i_h, i_w,  top_);
		}
}
...
int 	net_conv_backward(t_layer_info*  layer_list,int layer_id, float* bottom, float* top)
{
	...
	if(layer_list[layer_id].train_inf.para_diff_way==e_para1_prune){
		float* para1_data_host=new float[conv_w_size];
		cuda_pull_array(layer_list[layer_id].train_inf.para1_diff, para1_data_host, conv_w_size);
		caffe_mul(conv_w_size, para1_data_host, layer_list[layer_id].train_inf.para_diff_mask, para1_data_host);
		cuda_push_array(layer_list[layer_id].train_inf.para1_diff, para1_data_host, conv_w_size);
		delete []para1_data_host;
	}else if(layer_list[layer_id].train_inf.para_diff_way==e_para_unchange){
		cuda_set(conv_w_size,0,layer_list[layer_id].train_inf.para1_diff);
		if(layer_list[layer_id].train_inf.en_bias){
...
#if 0
	if(layer_id==2){
		print_gpu_data(layer_list[layer_id].train_inf.para1_diff, conv_w_size); 
...
int net_conv_backward_batch(t_layer_info*  layer_list,int layer_id,int batch_sz)
{
	int bottom_size=layer_list[layer_id].in_dim[0][0]*layer_list[layer_id].in_dim[0][1]*layer_list[layer_id].in_dim[0][2];
	int top_size=layer_list[layer_id].out_dim[0]*layer_list[layer_id].out_dim[1]*layer_list[layer_id].out_dim[2];
	for(int i=0;i<batch_sz;i++){
		float* bottom_=layer_list[layer_id].input_buf[0]+i*3*bottom_size;			
		float* top_=layer_list[layer_id].output_buf+i*3*top_size;
		net_conv_backward( layer_list,layer_id, bottom_, top_);
		}
}
